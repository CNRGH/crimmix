...
print("CIMLR")
CIMLR_results <- liver_filter %>% IntMultiOmics(method="CIMLR", K=4)
saveRDS(CIMLR_results, file=file.path(sprintf("inst/extdata/TCGA/Liver/CIMLR_res.rds")))

print("LRAcluster")
LRAcluster_results <- liver_filter %>% IntMultiOmics(method="LRAcluster", K=4, type=c("gaussian", "binary", "gaussian","gaussian"))
saveRDS(LRAcluster_results, file=file.path(sprintf("inst/extdata/TCGA/Liver/LRAcluster_res.rds")))

print("PINSPLUS")
PINSPLUS_results <- liver_filter %>% IntMultiOmics(method="PINSPlus", K=4)
saveRDS(PINSPLUS_results, file=file.path(sprintf("inst/extdata/TCGA/Liver/PINSPLUS_res.rds")))

print("Consensus Clustering")
ConsensusClustering_results <- liver_filter %>% IntMultiOmics(method="ConsensusClustering", K=4)
saveRDS(ConsensusClustering_results, file=file.path(sprintf("inst/extdata/TCGA/Liver/ConsensusClustering_res.rds")))
